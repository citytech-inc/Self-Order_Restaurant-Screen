...
const SalesAnalysis : React.FC = () => {
    ...
    const salesDetailData = {"純売上": 19800, "純売上(税別)": 18000, "消費税": 1800, "総売上": 18000, "値引き": 0, "原価": 4500, "粗利益": 13500, "販売費": 11500, "営業利益": 2000, "客数": 22, "価格変動": 2.5}
    const startDate = [2021,10,1]
    const Today = new Date();
    const DayName = ["日","月","火","水","木","金","土"]
    const YearMonthList: {[key: string]: number[]} = Object()
    for(let i = startDate; i[0] < Number(format(Today, 'yyyy')) || (i[0] === Number(format(Today, 'yyyy')) && i[1] <= Number(format(Today, 'MM')));){
    ...
    const [selectedYear, setSelectedYear] = useState<Number>(Number(format(Today, 'yyyy')));
    const [selectedMonth, setSelectedMonth] = useState<Number>(Number(format(Today, 'MM')));
    const [selectedDay, setSelectedDay] = useState<String>(DayName[Number(format(Today, 'e'))]);
    const dateList : Date[] = [];
    for(let i = 1; i < 8; i++){
    ...
    const SalesDataSet = {
        ...
        datasets: [{
            ...
            backgroundColor : Object.keys(SalesPerHour).map((key, index) => (Number(key)===selectedHour ? "rgba(255, 240, 180, 1)" : SalesPerHour[Number(key)].priceClass==="normal" ? "rgba(217, 217, 217, 1)" : "rgba(153, 153, 153, 1)")),
            borderRadius : 5, 
        }]
    }
    const options: {} = {
    ...
    return (
        ...
        <div className="display-option">
            <div className="display-span-option">
                ...
                <>
                <div className="display-week-select">{format(selectedDate, 'M月')}第{Math.floor((selectedDate.getDate() - Number(format(selectedDate, 'e')) + 5)/7)+1}週</div>
                <DatePicker
                    selected={selectedDate}
                    onChange={handleDateChange}
                    dateFormat={customDateFormat(selectedDate)} // 日付の表示形式を指定
                ...
                <>
                <select className="display-year-select" value={String(selectedYear)} onChange={(e) => setSelectedYear(Number(e.target.value))}>
                    {Object.keys(YearMonthList).map((key, index) => (
                        <option value={key}>{key}年</option>
                    ))}
                </select>
                <select className="display-month-select" value={String(selectedMonth)} onChange={(e) => setSelectedMonth(Number(e.target.value))}>
                ...
                <>
                <select className="display-day-select" value={String(selectedDay)} onChange={(e) => setSelectedDay(e.target.value)}>
            ...
            <div className="graph-and-calendar">
                    ...
                    <div className="table-col">
                        <div className="table-hour-col">
                            {Object.keys(SalesPerHour).map((key, index) => (
                                index < Object.keys(SalesPerHour).length/2 && <div className="table-text" style={{height: String(100/(Object.keys(SalesPerHour).length/2))+"%", backgroundColor: Number(key)===selectedHour ? "#FFF0B4" : "rgba(0,0,0,0)"}}>{key}時</div>
                            ))}
                        </div>
                        <div className="table-sales-col">
                            {Object.keys(SalesPerHour).map((key, index) => (
                                index < Object.keys(SalesPerHour).length/2 && <div className="table-text" style={{height: String(100/(Object.keys(SalesPerHour).length/2))+"%", backgroundColor: Number(key)===selectedHour ? "#FFF0B4" : "rgba(0,0,0,0)"}}>{SalesPerHour[Number(key)].sales.toLocaleString()}円</div>
                            ))}
                        </div>
                    </div>
                    <div className="table-col">
                        <div className="table-hour-col">
                            ...
                            {Object.keys(SalesPerHour).length%2 === 1 && <div className="table-text" style={{height: String(100/(Object.keys(SalesPerHour).length/2))+"%"}}>　</div>}
                        </div>
                        <div className="table-sales-col">
                            {Object.keys(SalesPerHour).map((key, index) => (
                                index >= Object.keys(SalesPerHour).length/2 && <div className="table-text" style={{height: String(100/(Object.keys(SalesPerHour).length/2))+"%", backgroundColor: Number(key)===selectedHour ? "#FFF0B4" : "rgba(0,0,0,0)"}}>{SalesPerHour[Number(key)].sales.toLocaleString()}円</div>
                ...
                <div className="sales-detail-data">
                    <div className="sales-detail-col">
                        <div className="row-1"><div className="row-name">純売上</div><div className="row-item">{salesDetailData["純売上"].toLocaleString()}円</div></div>
                        <div className="row-2"><div className="row-name">純売上(税別)</div><div className="row-item">{salesDetailData["純売上(税別)"].toLocaleString()}円</div></div>
                        <div className="row-1"><div className="row-name">消費税</div><div className="row-item">{salesDetailData["消費税"].toLocaleString()}円</div></div>
                        <div className="row-2"><div className="row-name">総売上</div><div className="row-item">{salesDetailData["総売上"].toLocaleString()}円</div></div>
                        <div className="row-1"><div className="row-name">値引き</div><div className="row-item">{salesDetailData["値引き"].toLocaleString()}円</div></div>
                        <div className="row-2"><div className="row-name">原価</div><div className="row-item">{salesDetailData["原価"].toLocaleString()}円</div></div>
                        <div className="row-1"><div className="row-name">粗利益</div><div className="row-item">{salesDetailData["粗利益"].toLocaleString()}円</div></div>
                        <div className="row-2"><div className="row-name">販売費</div><div className="row-item">{salesDetailData["販売費"].toLocaleString()}円</div></div>
                        <div className="row-1"><div className="row-name">営業利益</div><div className="row-item">{salesDetailData["営業利益"].toLocaleString()}円</div></div>
                    </div>
                    <div className="sales-detail-col">
                        <div className="row-3"><div className="row-name">客数</div><div className="row-item">{salesDetailData["客数"].toLocaleString()}人</div></div>
                        <div className="row-3"><div className="row-name">価格変動</div><div className="row-item">{salesDetailData["価格変動"].toLocaleString()}％</div></div>
                        <div className="row-3"><div className="row-name"></div><div className="row-item"></div></div>
                        <div className="row-3"><div className="row-name"></div><div className="row-item"></div></div>
                        <div className="row-3"><div className="row-name"></div><div className="row-item"></div></div>
                        <div className="row-3"><div className="row-name"></div><div className="row-item"></div></div>
                        <div className="row-3"><div className="row-name"></div><div className="row-item"></div></div>
